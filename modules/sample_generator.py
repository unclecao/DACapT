...
def gen_samples(generator, bbox, n, img_size, overlap_range=None, scale_range=None):
...
class SampleGenerator():
    def __init__(self, type, trans_f=1, scale_f=1, aspect_f=None, valid=False):
        self.type = type
    ...
    def __call__(self, bb, n, img_size):
        ...
        if self.type=='gaussian':
            ...
            m = int(2*np.sqrt(n))
            xy = np.dstack(np.meshgrid(np.linspace(0,1,m),np.linspace(0,1,m))).reshape(-1,2)# dstack equals to np.concatenate(, axis=2)
        ...
        if self.valid:
            samples[:,:2] = np.clip(samples[:,:2], samples[:,2:]/2, img_size-samples[:,2:]/2-1) #Not exceed the boundary 
        else:
